...
pipeline {
  ...
  options {
    ...
    gitlabBuilds(builds: ['kundb-mariadb-ci'])
  }
  parameters {
  ...
  stages {
    stage('checkout') {
      steps {
        waitDocker {}
        updateGitlabCommitStatus(name: 'kundb-mariadb-ci', state: 'pending')
        checkout scm
        // checkout scm: [$class: 'GitSCM', extensions: [[$class: 'CheckoutOption', timeout: 30]]]
      }
    }

    stage('compile x86_64 mariadb') {
      steps {
        sh """#!/bin/bash -ex
          ...
          export IMG_NAME=\${DockerRegistry}/\${DOCKER_PROD_NS}/x86_64/mariadb:${params.RELEASE_TAG}
    ...
    stage('compile arm64 mariadb') {
      steps {
        updateGitlabCommitStatus(name: 'kundb-mariadb-ci', state: 'running')
        sh """#!/bin/bash -ex
          docker login -u \$DOCKER_HARBOR_USR -p \$DOCKER_HARBOR_PSW \$DockerRegistry
          ...
          export IMG_NAME=\${DockerRegistry}/\${DOCKER_PROD_NS}/aarch64/mariadb:${params.RELEASE_TAG}
          docker build --label CODE_REVISION=\${REV} \
            --label BRANCH=${params.RELEASE_TAG} \
            --label COMPILE_DATE=\$(date +%Y%m%d-%H%M%S) \
  ...
  post {
    success {
      updateGitlabCommitStatus name: 'kundb-mariadb-ci', state: 'success'
    }
    failure {
      updateGitlabCommitStatus name: 'kundb-mariadb-ci', state: 'failed'
    }
    always {
      script {
        sendMail2 {
